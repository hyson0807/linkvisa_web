...
### DA 폰트 크기 0 (auto-size) 문제
- 증상: `신원보증서`, `전문인력 고용추천서`, `거주숙소 확인서`의 텍스트가 PDF에 표시되지 않음
- 원인: 세 템플릿의 DA가 `/Helv 0 Tf` — 폰트 크기 0(auto-size)인데, `getDefaultFontSize()`가 이를 리터럴 0으로 반환 → `maxSize=0` → fit 루프 미실행 → `minSize(2pt)` 폴백
- 해결: `getDefaultFontSize()`에서 `fontSize > 0` 조건 추가, 0은 `undefined`로 처리하여 `DEFAULT_MAX_FONT_SIZE(10)` 사용
- 교훈: 새 PDF 템플릿 추가 시 DA의 폰트 크기가 0인지 확인 필요. 0이면 auto-size이므로 기본 상한값을 적용해야 함
